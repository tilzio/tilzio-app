...
export function parseContributes(raw: unknown): Contributions {
  ...
  return {
    ...
    panels: asArray(c.panels).map(withId).filter((o): o is Record<string, unknown> => o !== null)
      .map((o): PanelDecl | null => {
        const location = o.location === 'right' ? 'right' as const : 'bottom' as const;
        const title = cleanStr(o.title);
        if (o.render === 'iframe') {
          const entry = cleanStr(o.entry, 256);
          return entry ? { id: o.id as string, location, title, render: 'iframe' as const, entry } : null;
        }
        return { id: o.id as string, location, title, render: 'widgets' as const };
      })
      .filter((p): p is PanelDecl => p !== null),
